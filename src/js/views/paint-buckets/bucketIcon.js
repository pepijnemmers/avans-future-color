/**
 * Returns the bucket icon with the given color or gradient
 * @param {string} color in hexcode
 * @param {boolean} isGradient is gradient icon
 * @returns {string} The bucket icon in SVG
 */
export function bucketIcon(color = "#787878", isGradient = false) {
    if (!color && !isGradient) {
        return bucketIconColor(color);
    }
    return isGradient ? bucketIconGradient() : bucketIconColor(color);
}

function bucketIconColor(color) {
    return `
